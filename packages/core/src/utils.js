...
const widgetMap = {
    boolean: {
        checkbox: "CheckboxWidget", radio: "RadioWidget", select: "SelectWidget", hidden: "HiddenWidget",
    }, string: {
        text: "TextWidget",
        ...
        select: "SelectWidget", checkboxes: "CheckboxesWidget", files: "FileWidget", hidden: "HiddenWidget",
    },
};
...
export function getDefaultRegistry() {
    return {
        fields, widgets, definitions: {}, rootSchema: {}, formContext: {},
    };
}
    ...
    switch (getSchemaType(schema)) {
        ...
        case "object":
            return Object.keys(schema.properties || {}).reduce((acc, key) => {
                ...
                let computedDefault = computeDefaults(schema.properties[key], (defaults || {})[key], rootSchema, (formData || {})[key], includeUndefinedValues);
            ...
            if (schema.minItems) {
                if (!isMultiSelect(schema, rootSchema)) {
                    ...
                    if (schema.minItems > defaultsLength) {
                        ...
                        const fillerEntries = fill(new Array(schema.minItems - defaultsLength), computeDefaults(fillerSchema, fillerSchema.defaults, rootSchema));
    ...
    return Object.keys(uiSchema)
        ...
        .reduce((options, key) => {
            ...
            if (key === "ui:widget" && isObject(value)) {
                ...
                return {
                    ...options, ...(value.options || {}), widget: value.component,
                };
            }
...
const resolveCondition = (schema, rootSchema, formData) => {
    let {
        if: expression, then, else: otherwise, ...resolvedSchemaLessConditional
    } = schema;

    const conditionalSchema = isValid(expression, formData, rootSchema) ? then : otherwise;
...
export function resolveSchema(schema, rootSchema = {}, formData = {}) {
    if (schema.hasOwnProperty("$ref")) {
        ...
        return {
            ...schema, allOf: schema.allOf.map(allOfSubschema => retrieveSchema(allOfSubschema, rootSchema, formData)),
        };
    } else {
...
function processDependencies(dependencies, resolvedSchema, rootSchema, formData) {
    ...
    for (const dependencyKey in dependencies) {
        ...
        const {
            [dependencyKey]: dependencyValue, ...remainingDependencies
        } = dependencies;
...
function withExactlyOneSubschema(schema, rootSchema, formData, dependencyKey, oneOf) {
    ...
    const {
        [dependencyKey]: conditionPropertySchema, ...dependentSubschema
...
export function toIdSchema(schema, id, rootSchema, formData = {}, idPrefix = "root", idSeparator = "_") {
    const idSchema = {
        $id: id || idPrefix,
    };
